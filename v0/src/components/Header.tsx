...
function DropdownMenu({ items }: { items: { label: string; href: string }[] }) {
  return (
    <div className="absolute top-full left-0 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.08)] rounded-[24px] border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 min-w-56 py-3 mt-1">
      {items.map((item) => (
        <Link
          key={item.href}
          href={item.href}
          className="block px-5 py-2.5 text-sm text-slate-600 hover:bg-[#ffffff] hover:text-[#5c54d1] whitespace-nowrap transition-colors"
        >
          {item.label}
        </Link>
      ))}
...
export default function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        {/* Top bar */}
        <div className="container mx-auto px-4 lg:px-8 py-4 flex justify-between items-center bg-white">
          ...
          <div className="hidden md:flex items-center gap-6">
            <a href="tel:0421608819" className="text-[#3f3a7d] font-medium text-sm hover:text-[#5c54d1] transition-colors flex items-center gap-2">
              <span>📞</span> {PHONE_DISPLAY}
            </a>
            <Link
              href="/book"
              className="bg-[#5c54d1] text-white px-6 py-2.5 rounded-full font-medium text-sm hover:bg-[#4a44a8] hover:shadow-md hover:-translate-y-0.5 transition-all duration-300"
            >
              Book a Free Discovery Call
            </Link>
            <Link
              href="/refer-a-client"
              className="border border-[#3f3a7d]/20 text-[#3f3a7d] px-6 py-2.5 rounded-full font-medium text-sm hover:bg-[#3f3a7d] hover:text-white transition-all duration-300"
            >
              Refer a Client
            </Link>
            <Link href="/zh" className="text-sm font-medium text-slate-500 hover:text-[#5c54d1] transition-colors ml-2">
              EN / 中文
            </Link>
          </div>
          ...
          >
            <span className={`block h-0.5 w-6 bg-[#3f3a7d] transition-all ${mobileOpen ? 'rotate-45 translate-y-2' : ''}`} />
            <span className={`block h-0.5 w-6 bg-[#3f3a7d] transition-all ${mobileOpen ? 'opacity-0' : ''}`} />
            <span className={`block h-0.5 w-6 bg-[#3f3a7d] transition-all ${mobileOpen ? '-rotate-45 -translate-y-2' : ''}`} />
          </button>
        </div>

        {/* Desktop nav bar */}
        <nav className="bg-[#3f3a7d] text-white hidden md:block" aria-label="Main navigation">
          <div className="container mx-auto px-4 lg:px-8 flex items-stretch">
            {/* Services dropdown */}
            <div className="relative group">
              <button className="px-5 py-3.5 hover:text-[#5c54d1] transition-colors flex items-center gap-1.5 text-sm font-medium h-full tracking-wide">
                Services <span className="text-[10px] opacity-70">▼</span>
              </button>
              <DropdownMenu items={services} />
            </div>

            <Link href="/how-it-works" className="px-5 py-3.5 hover:text-[#5c54d1] transition-colors text-sm font-medium tracking-wide">
              How It Works
            </Link>
            ...
            <Link href="/about" className="px-5 py-3.5 hover:text-[#5c54d1] transition-colors text-sm font-medium tracking-wide">
              About
            </Link>
            ...
            <Link href="/contact" className="px-5 py-3.5 hover:text-[#5c54d1] transition-colors text-sm font-medium tracking-wide">
              Contact
            </Link>
          </div>
        </nav>
          ...
          <nav className="md:hidden bg-white border-t border-slate-100 shadow-xl" aria-label="Mobile navigation">
            <div className="px-5 py-6 space-y-4">
              ...
              <Link
                ...
                className="block w-full text-center bg-[#5c54d1] text-white px-5 py-3.5 rounded-full font-medium shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
              >
                Book a Free Discovery Call
              </Link>
              <Link
                href="/refer-a-client"
                onClick={() => setMobileOpen(false)}
                className="block w-full text-center border border-[#3f3a7d]/20 text-[#3f3a7d] px-5 py-3.5 rounded-full font-medium"
              >
                Refer a Client
              </Link>
              <a href="tel:0421608819" className="block text-center text-[#3f3a7d] font-medium py-2 flex items-center justify-center gap-2">
                <span>📞</span> {PHONE_DISPLAY}
              </a>
              ...
              <button
                ...
                className="flex justify-between w-full text-[#3f3a7d] font-medium py-2.5"
              >
              ...
              {mobileServices && (
                <div className="pl-4 space-y-3 mb-2">
                  {services.map((s) => (
                    <Link key={s.href} href={s.href} onClick={() => setMobileOpen(false)} className="block text-sm text-slate-600 py-1 hover:text-[#5c54d1]">
                      {s.label}
                    </Link>
                  ))}
                </div>
              ...
              <Link href="/how-it-works" onClick={() => setMobileOpen(false)} className="block text-[#3f3a7d] font-medium py-2.5">
                How It Works
              </Link>
              ...
              {mobileFunding && (
                <div className="pl-4 space-y-3 mb-2">
                  {funding.map((f) => (
                    <Link key={f.href} href={f.href} onClick={() => setMobileOpen(false)} className="block text-sm text-slate-600 py-1 hover:text-[#5c54d1]">
                      {f.label}
                    </Link>
                  ))}
                </div>
              ...
              <Link href="/about" onClick={() => setMobileOpen(false)} className="block text-[#3f3a7d] font-medium py-2.5">
                About
              </Link>
              ...
              {mobileAreas && (
                <div className="pl-4 space-y-3 mb-2">
                  {areas.map((a) => (
                    <Link key={a.href} href={a.href} onClick={() => setMobileOpen(false)} className="block text-sm text-slate-600 py-1 hover:text-[#5c54d1]">
                      {a.label}
                    </Link>
                  ))}
                </div>
              ...
              <Link href="/contact" onClick={() => setMobileOpen(false)} className="block text-[#3f3a7d] font-medium py-2.5">
                Contact
              </Link>
