...
> **Homepage section order:** Differentiators-first (why-choose-us early)
> **Visual reference:** `_temp/v1/` — inspired by Pop Online Speech Therapy (playful, purple/green/yellow)
...
You are building a complete website for "Shine and Speak," a mobile speech pathology practice in Sydney, run by speech pathologist **Shine Teoh**. Build the site inside the `v1/` folder (this folder).
...
Do NOT copy the competitor site's colour scheme (Pop uses purple/green/yellow). Adapt their layout patterns to Shine and Speak's navy/gold/white palette.
...
- Source a warm, natural-looking royalty-free image from Unsplash (search: "speech therapy child", "therapist working with child", "child communication", "parent child learning") and include an HTML comment `<!-- REPLACE: Professional photo of Shine Teoh working with a child -->`
...
- The headshot thumbnail at `_temp/assets/shine-teoh-headshot-thumbnail.webp` can be used as a reference for the therapist's appearance, but is low-resolution — mark it for replacement with a professional photo
...
Look at the competitor website screenshots in `_temp/v1/` (v1a.png through v1g.png). These are from **Pop Online Speech Therapy**, an Australian telehealth speech therapy practice.

**What to take from these screenshots:**
- Bold, confident layout patterns with strong visual hierarchy
- Dual CTA placement in the hero (discovery call + start now)
- Stats/numbers bar as social proof
- "Why choose us" differentiators section with bullet points alongside imagery
- 3-step "How it works" with numbered cards
- Full-width CTA bands between content sections
- Services displayed with icons and category labels
- Footer with multiple columns of links
- Floating "Book a Free Discovery Call" button (bottom-right)

**What NOT to copy:**
- Their colour scheme (purple/green/yellow) — use Shine and Speak's navy/gold/white
- Their team photo sections (they have 85+ therapists; Shine and Speak is a solo practice)
- Their "Careers" and "Join the family" recruitment sections
- Their Instagram feed embed layout
- Any accordion or collapsed content patterns

### Typography

Choose a warm, approachable yet confident font pairing. Pop uses a decorative serif for headings — you may take inspiration from that contrast approach (display heading font + clean body font) but choose fonts that feel right for Shine and Speak's brand. Google Fonts recommended.
...
3. **Differentiators** — 4 icon blocks FIRST: "We come to you", "Bilingual", "NDIS/Medicare/Private", "Evidence-based" (Section 3, Section 6)
4. **Mobile advantage** — "Therapy where your child is most comfortable" with 4 delivery modes (Section 3, Section 5)
5. **Condition cards** — 6 cards linking to SEO URLs (Section 3, Section 3)
6. **How it works** — 3 steps with CTA (Section 3, Section 7)
7. **Bilingual callout** — English + Mandarin, link to `/zh/` (Section 3, Section 4)
8. **Meet the therapist** — Shine Teoh's bio with headshot placeholder, credentials, CTA to `/about` (Section 3, Section 8)
9. **Social proof** — Google Reviews placeholder with interim credential badges (Section 3, Section 9)
...
Build the complete site now. Make it visually polished and professional, drawing layout inspiration from the `_temp/v1/` screenshots while using Shine and Speak's own brand identity. Prioritise SEO architecture correctness, then visual quality.
